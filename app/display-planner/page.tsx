...
function KpiCard({ item }: { item: KpiItem }) {
  ...
  return (
    <div className="rounded-xl p-4 flex flex-col gap-2" style={{ backgroundColor: item.color + '12', border: `1px solid ${item.color}30` }}>
      <div className="text-xs font-bold uppercase tracking-widest" style={{ color: item.color }}>{item.title}</div>
      ...
      {item.type === 'countdown' && (
        <>
          <span className="text-3xl font-black tabular-nums" style={{ color: item.color }}>
    ...
    <div className={`flex flex-col rounded-xl overflow-hidden flex-1 min-w-0 ${isToday ? 'ring-2 ring-blue-500 ring-offset-2 ring-offset-[#0a0f1e]' : ''}`}
      ...
      <div className={`px-3 py-2 flex items-center justify-between flex-shrink-0 ${isToday ? 'bg-blue-600' : 'bg-white/5'}`}>
        <div className="flex items-center gap-2">
          <span className={`text-xs font-black uppercase tracking-widest ${isToday ? 'text-blue-100' : 'text-gray-400'}`}>{dayName}</span>
          <span className={`text-xl font-black ${isToday ? 'text-white' : 'text-gray-300'}`}>{dayNum}</span>
        </div>
        {events.length > 0 && (
      ...
      <div className="flex-1 overflow-hidden p-1.5 space-y-1">
        {sorted.length === 0 ? (
          ...
          sorted.map(ev => (
            <div key={ev.id}
              className="rounded-lg px-2 py-1.5 flex flex-col gap-0.5 overflow-hidden"
              ...
              {ev.notes && (
                <div className="text-[9px] text-gray-500 truncate">{ev.notes}</div>
              )}
            </div>
          ))
        )}
...
// ─── Main Display Page ────────────────────────────────────────────────────────
export default function DisplayPlannerPage() {
  ...
  const load = useCallback(() => {
    fetch(`/api/planner?week=${weekParam}`)
      .then(r => r.json()).then(setEvents).catch(() => {});
    fetch('/api/kpi')
      .then(r => r.json()).then(setKpis).catch(() => {});
  }, [weekParam]);

  // Reload every 30s, auto-rotate view every 20s if KPIs exist
  useEffect(() => {
    load();
  ...
  // Count today's upcoming events
  const todayEvents = events.filter(e => e.date === todayStr && !e.completed);
  const nowH = new Date().getHours() + new Date().getMinutes() / 60;
  const upcomingToday = todayEvents.filter(e => parseTime(e.end_time) > nowH);
    ...
    <div className="w-screen h-screen overflow-hidden flex flex-col"
      ...
      <div className="flex-shrink-0 flex items-center justify-between px-8 py-4 border-b" style={{ borderColor: '#1e2d4a' }}>
        <div className="flex items-center gap-6">
          ...
          {upcomingToday.length > 0 && (
            <div className="flex items-center gap-2 bg-blue-600/20 border border-blue-500/30 rounded-full px-4 py-1.5">
              <div className="w-2 h-2 rounded-full bg-blue-400 animate-pulse" />
              <span className="text-sm font-bold text-blue-300">{upcomingToday.length} upcoming today</span>
            </div>
          )}
        </div>
      ...
      {kpis.length > 0 && (
        <div className="flex-shrink-0 flex items-center justify-center gap-2 py-2">
          <button onClick={() => setView('week')}
            className={`w-2 h-2 rounded-full transition-all ${view === 'week' ? 'bg-blue-400 w-6' : 'bg-gray-700'}`} />
          <button onClick={() => setView('kpi')}
            className={`w-2 h-2 rounded-full transition-all ${view === 'kpi' ? 'bg-blue-400 w-6' : 'bg-gray-700'}`} />
        </div>
      )}
      ...
      <div className="flex-shrink-0 flex items-center justify-between px-8 py-2 border-t" style={{ borderColor: '#1e2d4a' }}>
        <div className="flex gap-4">
          {['job','meeting','deadline','travel','admin'].map(cat => (
            <div key={cat} className="flex items-center gap-1.5">
              <div className="w-2 h-2 rounded-full" style={{ backgroundColor: { job:'#3b82f6', meeting:'#8b5cf6', deadline:'#ef4444', travel:'#f59e0b', admin:'#10b981' }[cat] ?? '#6b7280' }} />
              <span className="text-[10px] font-bold uppercase tracking-wider text-gray-500">{CATEGORY_LABELS[cat]}</span>
            </div>
          ))}
        </div>
        <div className="text-[10px] text-gray-700 uppercase tracking-widest">Auto-refreshes every 30s</div>
      </div>
    </div>
  );
}
